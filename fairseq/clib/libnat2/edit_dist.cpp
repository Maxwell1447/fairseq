...
list<long> filterRedundancy(bool* masked_k_best, const long& k, const long& seq_len) {
  ...
  for (long j = 0; j < k; ++j) {
    ...
    if (found_no_match) filter.push_back(j);
  }
...
void recursiveCoverSearch(
  ...
  if (current_dim == dim) {
    long score = 0;
    for (long i = 0; i < seq_len; ++i) score += (long)current_cover[i];
    if (score > max_score) {
      max_score = score;
...
void getOpsFromSingle(
  ...
  long max_score = 0;
  bool max_cover[seq_len] = {0};
  bool current_cover[seq_len] = {0};
  ...
  for (long i = 0; i < n; ++i) {
    ...
    for (vector<long> const&idx : all_indexations.at(i).at(j)) {
      // cout << i << "-" << idx.at(0) << "   " << (i * seq_len + idx.at(0)) << "\n";
      del[i * seq_len + idx.at(0)] = 1;
      ins[i * seq_len + cpt_index] = idx.at(1) - cpt_ins - 1;
      // cmb[idx.at(1) * n + i] = 1;
...
class EditOpsBatch {
  public:
    ...
    EditOpsBatch(
      ...
      ins = torch::zeros({bsz, n, seq_len}, options);
